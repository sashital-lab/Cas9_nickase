...
cat ${file} | bioawk -c fastx '{print "@"$name;print revcomp($seq); print "+"; print revcomp($qual)}' | sed -n '2~4'p |\
sed 's/.*GTGCTTCAGGTTTA\(.\{20\}\)GGGGGTGTCAAGCTCGGA.*/\1/g' |\
awk 'length($1)==20' |\
...
#${file%.*}_extracted_cas.jf ${file%.*}_counts_dumps.fa
